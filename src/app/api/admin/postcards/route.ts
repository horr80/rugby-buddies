...
import { postcardEmailTemplate, sendEmail } from "@/lib/email";
...
export async function POST(request: Request) {
  try {
    ...
    const { heading, message, recipientFilter, filterId } = body as {
      heading: string;
      message: string;
    ...
    if (!heading || !message || !recipientFilter) {
      return NextResponse.json({ error: "heading, message, and recipientFilter are required" }, { status: 400 });
    }
    ...
    const subject = String(heading);
    let sent = 0;
    const errors: string[] = [];

    for (const r of recipients) {
      const name = `${r.firstName} ${r.lastName}`.trim() || "Parent";
      const html = postcardEmailTemplate(name, String(message), String(heading));
      try {
    ...
    console.error(e);
    return NextResponse.json({ error: "Failed to send postcards" }, { status: 500 });
  }
}
